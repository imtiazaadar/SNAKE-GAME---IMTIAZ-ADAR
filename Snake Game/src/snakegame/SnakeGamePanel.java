...
public class SnakeGamePanel extends JPanel implements ActionListener{
	...
	private boolean highscorefourdigit = false;
	...
	public void CheckAppleEaten() {
		if(appleX == x[0] && appleY == y[0]) {
			...
			else if(score > 100) delay -= 0.008;
			else if(score > 250) delay -= 1.000;
			else if(score > 500) delay -= 5;
			else if(score > 800) delay -= 15;
			else if(score > 1000) delay -= 25;
			else if(score > 1500) delay -= 50;
			else if(score > 2000) delay -= 60;
			if(highscore < score) highscore = score;
			if(Integer.toString(highscore).length() >= 4) highscorefourdigit = true;
		}
	}
	...
	public void ScoreSet(Graphics g) {
		...
		if(highscorefourdigit) g.drawString("SCORE : " + score, ((width - font.stringWidth("SCORE : " + score)) / 2) + 120, 64);
